...
from opendbc.car.mazda.values import CAR, DBC, G46L_RADAR_FW, LKAS_LIMITS, STEER_TO_ZERO_EPS_FW, MazdaFlags, MazdaSafetyFlags, TorqueInterceptorControllerParams
...
# --- Fork: flag ABI, torque interceptor enablement, TI wire layout ---

def test_mazda_flag_namespaces_are_noncolliding():
  assert int(MazdaFlags.GEN1) == 1
  assert int(MazdaFlags.STEER_TO_ZERO_EPS) == 2
  assert int(MazdaFlags.LEGACY_FW_EPS) == 4
  assert int(MazdaFlags.G46L_RADAR) == 8
  assert int(MazdaFlags.TORQUE_INTERCEPTOR) == 16
  assert int(MazdaFlags.EPS_HW) == 6
  assert int(MazdaSafetyFlags.LONG) == 1
  assert int(MazdaSafetyFlags.STEER_TO_ZERO_EPS) == 2
  assert int(MazdaSafetyFlags.LEGACY_FW_EPS) == 4
  assert int(MazdaSafetyFlags.TORQUE_INTERCEPTOR) == 8
  assert not (MazdaFlags.EPS_HW & MazdaFlags.TORQUE_INTERCEPTOR)


def _setup_ti(CP, CP_SP, enabled=True):
  setup_interfaces(CarInterface, CP, CP_SP, [{"TorqueInterceptorEnabled": enabled}])


@pytest.mark.parametrize("candidate,version,enabled,intrinsic", [
  (CAR.MAZDA_CX5, STOCK_CX5_EPS_FW, False, False),
  (CAR.MAZDA_CX5, SWAPPED_EPS_FW, False, False),
  (CAR.MAZDA_CX5, STOCK_CX5_EPS_FW, True, False),
  (CAR.MAZDA_CX5, SWAPPED_EPS_FW, True, False),
  (CAR.MAZDA_CX8_2022, None, False, True),
])
@pytest.mark.parametrize("alpha_long", [False, True])
def test_ti_setup_preserves_eps_and_long(candidate, version, enabled, intrinsic, alpha_long):
  fw = eps_fw(version) if version is not None else []
  cp = car_params(candidate, alpha_long=alpha_long, car_fw=fw)
  sp = car_params_sp(cp, candidate, alpha_long=alpha_long, car_fw=fw)
  before = (cp.flags, cp.safetyConfigs[0].safetyParam,
            cp.dashcamOnly, cp.minSteerSpeed, cp.steerAtStandstill)
  long_before = cp.openpilotLongitudinalControl
  setup_interfaces(CarInterface, cp, sp, [{"TorqueInterceptorEnabled": enabled}])
  ti = enabled or intrinsic
  assert cp.flags == before[0] | (16 if ti else 0)
  assert cp.safetyConfigs[0].safetyParam == before[1] | (8 if ti else 0)
  assert cp.openpilotLongitudinalControl == long_before
  if ti:
    assert not cp.dashcamOnly
    assert cp.minSteerSpeed == 0
    assert cp.steerAtStandstill
    assert TorqueInterceptorControllerParams(cp).STEER_MAX == 600
  else:
    assert (cp.dashcamOnly, cp.minSteerSpeed, cp.steerAtStandstill) == before[2:]


class TestMazdaTorqueInterceptorParams:
  def test_cx8_runs_controller_module_default_gains(self):
    # The per-car capnp gain channel was removed 2026-08-28: every happy owner mile
    # (v2, NNLC) ran module defaults 0.8/0.15, and the channel was silently dead in
    # the field. kp/ki unset = latcontrol_torque.py module constants.
    CP = car_params(CAR.MAZDA_CX8_2022)
    assert CP.lateralTuning.torque.deprecated.kp == 0.0
    assert CP.lateralTuning.torque.deprecated.ki == 0.0
    assert car_params(CAR.MAZDA_CX5_2022).lateralTuning.torque.deprecated.kp == 0.0

  def test_enabled_param_composes_flags_and_long_safety(self):
    CP = car_params(CAR.MAZDA_CX5_2022, alpha_long=True)
    CP_SP = car_params_sp(CP, CAR.MAZDA_CX5_2022, alpha_long=True)
    _setup_ti(CP, CP_SP)
    assert CP.flags & MazdaFlags.GEN1
    assert CP.flags & MazdaFlags.STEER_TO_ZERO_EPS
    assert CP.flags & MazdaFlags.TORQUE_INTERCEPTOR
    assert CP.safetyConfigs[0].safetyParam == (
      MazdaSafetyFlags.LONG | MazdaSafetyFlags.STEER_TO_ZERO_EPS | MazdaSafetyFlags.TORQUE_INTERCEPTOR)
    assert CP.minSteerSpeed == 0
    assert CP.steerAtStandstill

  def test_non_gen1_configuration_is_rejected(self):
    CP = car_params(CAR.MAZDA_CX5, car_fw=eps_fw(STOCK_CX5_EPS_FW))
    CP_SP = car_params_sp(CP, CAR.MAZDA_CX5, car_fw=eps_fw(STOCK_CX5_EPS_FW))
