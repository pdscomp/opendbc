...
from opendbc.car.mazda.carstate import CAM_LANEINFO_FRESH_FRAMES, STOCK_CTS_ALERT_FRAMES, TI_FEEDBACK_FRESH_FRAMES
...
# --- Fork: torque interceptor, follow-distance buttons, alpha-long takeover states ---

def ti_interface(alpha_long=False, candidate=CAR.MAZDA_CX5_2022):
...
def make_ti_feedback(torque=0, **overrides):
...
def feed_ti(CI, frames=1, torque=0, speed_kph=None, **overrides):
  ...
  for i in range(1, frames + 1):
    msgs = [make_ti_feedback(torque, **overrides)]
    if speed_kph is not None:
      msgs.append(pkr.make_can_msg("WHEEL_SPEEDS", 0, {"FL": speed_kph, "FR": speed_kph, "RL": speed_kph, "RR": speed_kph}))
...
def test_invalid_lkas_gate_is_stock_gen1_only():
  ...
  stock_ret, _ = car_interface(alpha_long=False, candidate=CAR.MAZDA_CX5).update([(20_000_000, [laneinfo])])
  stz_ret, _ = car_interface(alpha_long=False).update([(20_000_000, [laneinfo])])
...
class TestDistanceButtons:
  def _events(self, distance_less, distance_more):
    ...
    for i, values in enumerate(((0, 0), (distance_less, distance_more))):
      msg = pkr.make_can_msg("CRZ_BTNS", 0, {"DISTANCE_LESS": values[0], "DISTANCE_MORE": values[1]})
...
class TestMazdaTorqueInterceptorState:
  def test_feedback_parser_is_ti_only_on_bus_one_without_global_liveness(self):
    assert Bus.body not in car_interface(alpha_long=False).can_parsers

    parser = ti_interface().can_parsers[Bus.body]
    assert parser.bus == 1
    assert parser.message_states[0x24A].ignore_alive

  def test_ti_feedback_gap_keeps_optional_body_parser_valid(self):
    CI = ti_interface()
    feed_ti(CI, speed_kph=50)
  ...
  def test_feedback_health_controls_temporary_fault(self, overrides, healthy):
    CI = ti_interface()
    ret = feed_ti(CI, speed_kph=50, **overrides)  # fault only faults at speed
    assert CI.can_parsers[Bus.body].can_valid
  ...
  def test_ti_not_ready_fault_matches_zero_command_boundary(self, speed_kph, fault):
    CI = ti_interface()
    ret = feed_ti(CI, speed_kph=speed_kph, STATE=1, VIOL=0x11)
    assert not CI.CS.ti_lkas_allowed
    assert ret.steerFaultTemporary is fault


class TestAlphaLongTakeoverStates:
  """The pending/initializing pair reports the never-established-ownership wait only; once the
  controller's claim plus the silence guard latch radar_was_silenced, neither can reappear."""

  def _feed(self, CI, i, speed_kph, acc_active, radar_alive):
    pkr = packer()
    msgs = [pkr.make_can_msg("PEDALS", 0, {"ACC_OFF": 0 if acc_active else 1,
                                           "ACC_ACTIVE": 1 if acc_active else 0}),
            pkr.make_can_msg("ENGINE_DATA", 0, {"SPEED": speed_kph}),
            pkr.make_can_msg("WHEEL_SPEEDS", 0, {
              "FL": speed_kph, "FR": speed_kph, "RL": speed_kph, "RR": speed_kph,
            })]
    if radar_alive:
      msgs.append(mazdacan.create_acc_command(pkr, 0, i, 0., long_active=False, acc_available=True))
  ...
  def test_takeover_states_track_setup_blockers_and_completion(self):
    ...
    for i in range(2):
      ret, ret_sp = self._feed(CI, i, speed_kph=0, acc_active=False, radar_alive=True)
    assert ret.standstill
    assert ret_sp.alphaLongTakeoverInitializing
    assert not ret_sp.alphaLongTakeoverPending

    ret, ret_sp = self._feed(CI, 2, speed_kph=20, acc_active=False, radar_alive=True)
    assert not ret.standstill
    assert ret_sp.alphaLongTakeoverPending
    assert not ret_sp.alphaLongTakeoverInitializing

    ret, ret_sp = self._feed(CI, 3, speed_kph=0, acc_active=True, radar_alive=True)
    assert ret.standstill
    assert ret_sp.alphaLongTakeoverPending
    assert not ret_sp.alphaLongTakeoverInitializing

    # the controller claims replacement traffic with its first synthetic frame
    CI.CS.radar_control_active = True
    end = 4 + int((CarControllerParams.STOCK_RADAR_GUARD_T + 0.5) / DT_CTRL)
    for i in range(4, end):
      ret, ret_sp = self._feed(CI, i, speed_kph=0, acc_active=False, radar_alive=False)
