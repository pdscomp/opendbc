...
class TorqueInterceptorControllerParams:
  ...
  def __init__(self, CP=None):
    ...
    if CP.flags & MazdaFlags.STEER_TO_ZERO_EPS or CP.carFingerprint == "MAZDA_CX8_2022":
      self.STEER_DELTA_UP = 12
    if CP.flags & MazdaFlags.STEER_TO_ZERO_EPS:
...
class MazdaFlags(IntFlag):
  ...
  # Fork: comma torque interceptor on the EPS line. Bit 16 because 2/4/8 carry EPS and radar
  # dialect meanings upstream; the old fork value 4 collided with LEGACY_FW_EPS.
  TORQUE_INTERCEPTOR = 16

  # Source-compatibility alias for fork callers written against the pre-split name.
  STEER_TO_ZERO = STEER_TO_ZERO_EPS
...
class MazdaSafetyFlags(IntFlag):
  ...
  LEGACY_FW_EPS = 4
  # Fork: enables the torque-interceptor TX/RX path and bus routing in panda safety. Bit 8
  # because the old fork value 2 collided with STEER_TO_ZERO_EPS.
